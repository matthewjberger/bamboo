...
| `skills.toml` | Optional `title` + `description` + `sort` (`"asc"` / `"desc"`) plus `items = [...]` and/or `top_items = [...]` (lists of strings) | Skills section: `top_items` render as filled gradient pills (highlighted), `items` render as muted outline pills. `sort` applies to both lists at once. |
